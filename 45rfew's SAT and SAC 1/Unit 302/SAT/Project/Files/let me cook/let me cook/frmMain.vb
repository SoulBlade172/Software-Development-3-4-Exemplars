...
' - Cleaned code
' - Bug fixes 
' 19/6/2023 - VERSION 1.0.6
' - Sort screen no longer opens epmty after sorting 
' 13/7/2023 - VERSION 1.1.7
' - SaveAll feature no longer displays saved message despite canceling 
' - Added option to remove data from all selected cells 
' - Small UI changes 
' - Fix search feature bugging out after opening and closing once 
' - Encryption LMAO
' - remove _INT keyword from search results 
...
Public Class frmMain
    Private Sub mainLoad(sender As Object, e As EventArgs) Handles MyBase.Load 'sets form size to maximun window size and adjusts main tab control to fit
        ...
        tbcMain.Height = Height - 2303
        tbcMain.Width = Width - 280
        tmrMain.Interval = 100
        tmrMain.Start()
        Dim items() As ToolStripMenuItem = {mnuNew, mnuOpen, mnuSave, mnuSearch, mnuSort, mnuSaveAll},
        Hkeys() As Keys = {Keys.N, Keys.O, Keys.S, Keys.F, Keys.R, Keys.S}, display = {"Ctrl+N", "Ctrl+O", "Ctrl+S", "Ctrl+F", "Ctrl+R", "Ctrl+Shift+S"}
        For i = 0 To items.Length - 1 'loop through menustrip items and assign hotkeys
            items(i).ShortcutKeys = Keys.Control Or Hkeys(i) 'assign the hotkey combination 
            items(i).ShortcutKeyDisplayString = display(i) 'assign the display string for the combination 
            If display(i) = "Ctrl+Shift+S" Then ' check if it matches to the saveAll item 
                items(i).ShortcutKeys = items(i).ShortcutKeys Or Keys.Shift 'add shift key to the combination 
            End If
        Next
    End Sub
    ...
    End Sub
    Private Sub remove(sender As Object, e As EventArgs) Handles btnRemove.Click
        invokeFunc(AddressOf classes.remove)
    End Sub
...
Public Class classes 'public class containing all related class function 
    ...
    Public Shared Sub updateCol() 'updates the header text of a column 
        ...
        End If
    End Sub
    Public Shared Sub remove() 'removes selected data from cells
        Dim grid = getClass()
        Try
            Dim selected As DataGridViewSelectedCellCollection = grid.SelectedCells
            If selected.Count <= 0 Then Exit Sub 'check if any cells are selected
            For Each cell As DataGridViewCell In selected 'iterate through the selected cells and clear the data within each cell
                If Not cell.Value Is Nothing AndAlso Not String.IsNullOrEmpty(cell.Value.ToString()) Then 'skip empty cells
                    cell.Value = Nothing
                End If
            Next
        Catch ex As Exception
...
Public Class files 'public class containing all file related functions 
    ...
    Public Shared Sub saveAll()
        ...
        End If
        Try
            Dim allSaved = True 'flag to track if all classes were saved
            For Each tab As TabPage In frmMain.tbcMain.TabPages 'iterates through each class 
                ...
                If classes.classes.ContainsKey(name) AndAlso classes.classes(name) = False Then 'if the value pair of the class matches to false saves the class in a new xml doc 
                    ...
                    Else
                        allSaved = False 'saving was canceled or error thrown
                    End If
                Else
                    files.save() 'appends data to xml file if it already exists 
                End If
            Next
            If allSaved Then globalFunctions.echo("All classes saved")
        Catch ex As Exception
        End Try
    End Sub
    'UNUSED CODE: 
    ...
    Public Shared Function getNumericCol(grid As DataGridView) As List(Of String) 'another attempt; this one replaced the columns but broke the sort
        Dim numCol As New List(Of String)()
        For Each column As DataGridViewColumn In grid.Columns
            ...
            Next
            If isNum Then
                numCol.Add(column.Name)
                MsgBox(column.Index)
                MsgBox(column.Name)
                column.ValueType = GetType(Integer)
                Dim colIndex As Integer = column.Index
                Dim colName As String = column.Name
                grid.Columns.RemoveAt(colIndex)
                Dim intCol As New DataGridViewTextBoxColumn()
                intCol.Name = colName
                intCol.HeaderText = colName
                intCol.ValueType = GetType(Integer)
                grid.Columns.Insert(colIndex, intCol)
                Exit For
            End If
        Next
        Return numCol
    ...
    Public Shared Function fetchRow(row As DataGridViewRow) As String 'gets the data of a class row 
        ...
        For Each cell As DataGridViewCell In row.Cells 'iterate over each cell in the row 
            If cell.Value IsNot Nothing Then 'check if the cell value is not null and add it to the list 
                Dim rowD As String = $"{grid.Columns(cell.ColumnIndex).Name.Split("_INT")(0)}: {cell.Value.ToString()}"
                data.Add(rowD)
            End If
        Next
...
Public Class stats 'public class that contains all related stat calculations - did more calculations here than all of maths this year lmao 
    ...
    Public Shared Sub stats()
        ...
        For Each cell In colArr
            ...
            End Try
        Next
        average = Math.Round(sum / size, 2) 'calculate avarage 
